...
{lib}: let
  ...
  getFormatterModule = name:
    ...
    then import formatterModules.${name}.path
    else throw "Unknown formatter module: ${name}";

  # Get formatter module for Nix based on configuration
  getNixFormatterModule = formatter:
    if formatter == "nixfmt-rfc-style"
    then getFormatterModule "nix-nixfmt"
    else getFormatterModule "nix";
  ...
  loadFormatterModules = enabledFormatters: nixFormatter: let
    moduleList =
      lib.optionals (enabledFormatters.nix or false) [
