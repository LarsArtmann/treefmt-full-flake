...
export class TerminalDashboard {
  ...
  private async drawOverviewView(): Promise<void> {
    ...
    const summaryData = [
      ['Metric', 'Value', 'Change'],
      ['Avg Format Time', this.formatTime(metrics.avgFormatTime), this.formatChange(metrics.trends.performanceChange)],
      ['Files Processed', metrics.totalFiles.toString(), this.formatChange(metrics.trends.usageChange)],
      ['Success Rate', this.formatPercent(metrics.successRate), this.formatChange(metrics.trends.qualityChange)],
  ...
  private async drawDetailedView(): Promise<void> {
    ...
    const fileData = [
      ...
      ...metrics.slowestFiles.slice(0, 10).map(f => [
  ...
  private drawHeader(): void {
    ...
    term.moveTo(2, 2);
    term(`View: ${this.currentView.toUpperCase()} | Range: ${this.config.timeRange}d | Updated: ${now}`);
